...
StructInfo:	.space	28	#Número dado em bytes: | Data-2 | QtdCombustível-2 | Preço-4 | Distancia-4 | NomePosto-16 |

.text
#----------- Inicializando -----------#
	and $s7,$s7,$zero	#"Seta" $s7 para 0 pois este contará quantos cadastros foram feitos	
#------------ Exibir Menu ------------#
Menu:
	li  $v0, 4
	la $a0, Cadastrar
...
Cadastro:
	...
	jal DateToEpoch 
	add $t1,$zero,$v0 # $t1 detêm do valor da EPOCH
	...
	la $s4,ReadString
	la $s3,StructInfo
	sw $t1,0($s3)	#Data e Qtd Comb. OK
	s.s $f0,4($s3)	#Preço do litro	OK
	sw $s1,8($s3)	#Km Atual	O
	addi $s3,$s3,0xc
	addi $t0,$zero,4
